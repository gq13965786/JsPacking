ape.programlib.particle = {
    ...
    createShaderDefinition: function (device, options) {
        ...
        var chunk = ape.shaderChunks;

        var vshader = "";
        var fshader = ape.programlib.precisionCode(device) + "\n";
        ...
        fshader += ape.programlib.gammaCode(options.gamma);
        fshader += ape.programlib.tonemapCode(options.toneMap);
        ...
        if (options.blend == ape.BLEND_NORMAL) {
            fshader += chunk.particle_blendNormalPS;
        } else if (options.blend == ape.BLEND_ADDITIVE) {
            fshader += chunk.particle_blendAddPS;
        } else if (options.blend == ape.BLEND_MULTIPLICATIVE) {
            fshader += chunk.particle_blendMultiplyPS;
        }
        fshader += chunk.particle_endPS;

        var attributes = ape.shaderChunks.collectAttribs(vshader);
