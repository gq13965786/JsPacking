...
ape.programlib.standard = {
    ...
    generateKey: function (options) {
        ...
        return ape.hashCode(key);
    },

    _correctChannel: function (p, chan) {
        if (ape._matTex2D[p] > 0) {
            if (ape._matTex2D[p] < chan.length) {
                return chan.substring(0, ape._matTex2D[p]);
            } else if (ape._matTex2D[p] > chan.length) {
                var str = chan;
                var chr = str.charAt(str.length - 1);
                var addLen = ape._matTex2D[p] - str.length;
    ...
    _getUvSourceExpression: function (transformPropName, uVPropName, options) {
        ...
        if (options.nineSlicedMode === ape.SPRITE_RENDERMODE_SLICED) {
            expression = "nineSlicedUv";
        } else if (options.nineSlicedMode === ape.SPRITE_RENDERMODE_TILED) {
    ...
    _nonPointShadowMapProjection: function (device, light, shadowCoordArgs) {
        if (!light._normalOffsetBias || light._isVsm) {
            if (light._type === ape.LIGHTTYPE_SPOT) {
    ...
    _vsAddBaseCode: function (code, device, chunks, options) {
        code += chunks.baseVS;
        if (options.nineSlicedMode === ape.SPRITE_RENDERMODE_SLICED ||
            options.nineSlicedMode === ape.SPRITE_RENDERMODE_TILED) {
     ...
     * @param  {ape.GraphicsDevice} device The graphics device
    ...
    createShaderDefinition: function (device, options) {
        ...
        if (options.shadingModel === ape.SPECULAR_PHONG) {
            ...
            options.fresnelModel = (options.fresnelModel === 0) ? ape.FRESNEL_SCHLICK : options.fresnelModel;
        }
        ...
        var shadowPass = options.pass >= ape.SHADER_SHADOW && options.pass <= 17;
                ...
                if (chunks.hasOwnProperty(p)) {
                    if (!options.chunks[p]) {
                        ...
                        if (chunk.indexOf("vertex_normal") >= 0) {
                            attributes.vertex_normal = ape.SEMANTIC_NORMAL;
                        }
                        if (chunk.indexOf("vertex_tangent") >= 0) {
                            attributes.vertex_tangent = ape.SEMANTIC_TANGENT;
                        }
                        if (chunk.indexOf("vertex_texCoord0") >= 0) {
                            attributes.vertex_texCoord0 = ape.SEMANTIC_TEXCOORD0;
                        }
                        if (chunk.indexOf("vertex_texCoord1") >= 0) {
                            attributes.vertex_texCoord1 = ape.SEMANTIC_TEXCOORD1;
                        }
                        if (chunk.indexOf("vertex_color") >= 0) {
                            attributes.vertex_color = ape.SEMANTIC_COLOR;
                        }
                        if (chunk.indexOf("vertex_boneWeights") >= 0) {
                            attributes.vertex_boneWeights = ape.SEMANTIC_BLENDWEIGHT;
                        }
                        if (chunk.indexOf("vertex_boneIndices") >= 0) {
                            attributes.vertex_boneIndices = ape.SEMANTIC_BLENDINDICES;
                        }
        ...
        if (!options.noShadow && !options.twoSidedLighting) {
            for (i = 0; i < options.lights.length; i++) {
                ...
                if (options.lights[i].castShadows) {
                    if (lightType === ape.LIGHTTYPE_DIRECTIONAL) {
                        code += "uniform mat4 light" + i + "_shadowMatrixVS;\n";
                        code += "uniform vec3 light" + i + "_shadowParamsVS;\n";
                        code += "uniform vec3 light" + i + (lightType === ape.LIGHTTYPE_DIRECTIONAL ? "_directionVS" : "_positionVS") + ";\n";
                        mainShadowLight = i;
                        break;
                    }
                }
        ...
        if (options.pass === ape.SHADER_DEPTH) {
        ...
        if (options.useInstancing) {
            attributes.instance_line1 = ape.SEMANTIC_TEXCOORD2;
            attributes.instance_line2 = ape.SEMANTIC_TEXCOORD3;
            attributes.instance_line3 = ape.SEMANTIC_TEXCOORD4;
            attributes.instance_line4 = ape.SEMANTIC_TEXCOORD5;
        ...
        for (i = 0; i < maxUvSets; i++) {
            if (useUv[i]) {
                attributes["vertex_texCoord" + i] = ape["SEMANTIC_TEXCOORD" + i];
                code += chunks["uv" + i + "VS"];
        ...
        if (options.skin) {
            attributes.vertex_boneWeights = ape.SEMANTIC_BLENDWEIGHT;
            attributes.vertex_boneIndices = ape.SEMANTIC_BLENDINDICES;
            code += ape.programlib.skinCode(device, chunks);
            code += "#define SKIN\n";
        ...
        var startCode = "";
        if (device.webgl2) {
            startCode = ape.programlib.versionCode(device);
        ...
        if (device.webgl2) {
            code += ape.programlib.versionCode(device);
        }
        ...
        code += options.forceFragmentPrecision ? "precision " + options.forceFragmentPrecision + " float;\n\n" : ape.programlib.precisionCode(device);

        if (options.pass === ape.SHADER_PICK) {
            // ##### PICK PASS #####
            ...
            code += ape.programlib.begin();
            if (options.alphaTest) {
                code += "   getOpacity();\n";
                code += "   alphaTest(dAlpha);\n";
            }
            code += "    gl_FragColor = uColor;\n";
            code += ape.programlib.end();
            return {
        ...
        } else if (options.pass === ape.SHADER_DEPTH) {
            // ##### SCREEN DEPTH PASS #####
            code += 'varying float vDepth;\n';
            code += varyings;
            ...
            // ##### SHADOW PASS #####
            var smode = options.pass - ape.SHADER_SHADOW;
            ...
            if (shadowType === ape.SHADOW_VSM32) {
                if (device.textureFloatHighPrecision) {
                    code += '#define VSM_EXPONENT 15.0\n\n';
                } else {
                    code += '#define VSM_EXPONENT 5.54\n\n';
                }
            } else if (shadowType === ape.SHADOW_VSM16) {
                code += '#define VSM_EXPONENT 5.54\n\n';
            }
            ...
            if (shadowType === ape.SHADOW_PCF3 && (!device.webgl2 || lightType === ape.LIGHTTYPE_POINT)) {
                code += chunks.packDepthPS;
            } else if (shadowType === ape.SHADOW_VSM8) {
            ...
            var isVsm = shadowType === ape.SHADOW_VSM8 || shadowType === ape.SHADOW_VSM16 || shadowType === ape.SHADOW_VSM32;

            if (lightType === ape.LIGHTTYPE_POINT || (isVsm && lightType !== ape.LIGHTTYPE_DIRECTIONAL)) {
            ...
            } else if (shadowType === ape.SHADOW_PCF3 || shadowType === ape.SHADOW_PCF5) {
        ...
        for (i = 0; i < options.lights.length; i++) {
            ...
            if (lightType === ape.LIGHTTYPE_DIRECTIONAL) {
                ...
                if (lightType === ape.LIGHTTYPE_SPOT) {
                    code += "uniform vec3 light" + i + "_direction;\n";
            ...
            if (light.castShadows && !options.noShadow) {
                ...
                if (lightType === ape.LIGHTTYPE_POINT) {
                ...
                if (light._isPcf && (device.webgl2 || device.extStandardDerivatives) && lightType === ape.LIGHTTYPE_SPOT) usePerspZbufferShadow = true;
            }
            if (light._cookie) {
        ...
        code += ape.programlib.gammaCode(options.gamma);
        code += ape.programlib.tonemapCode(options.toneMap);
        code += ape.programlib.fogCode(options.fog);
        ...
        if (options.blendType !== ape.BLEND_NONE || options.alphaTest || options.alphaToCoverage) {
            code += this._addMap("opacity", "opacityPS", options, chunks);
        }
        ...
        if (options.useSpecular && (lighting || reflections)) {
            ...
            if (options.fresnelModel > 0) {
                if (options.fresnelModel === ape.FRESNEL_SIMPLE) {
                    code += chunks.fresnelSimplePS;
                } else if (options.fresnelModel === ape.FRESNEL_SCHLICK) {
                    code += chunks.fresnelSchlickPS;
                } else if (options.fresnelModel === ape.FRESNEL_COMPLEX) {
        ...
        if (useAo) {
            ...
            if (options.occludeSpecular) {
                if (options.occludeSpecular === ape.SPECOCC_AO) {
        ...
        if (options.useSpecular) {
            if (lighting) code += options.shadingModel === ape.SPECULAR_PHONG ? chunks.lightSpecularPhongPS : chunks.lightSpecularBlinnPS;
        ...
        if (lighting || reflections) {
            ...
            for (i = 0; i < options.lights.length; i++) {
                ...
                if (lightType === ape.LIGHTTYPE_DIRECTIONAL) {
                    ...
                    if (light._cookie) {
                        if (lightType === ape.LIGHTTYPE_SPOT && !light._cookie._cubemap) {
                            usesCookie = true;
                            usesCookieNow = true;
                        } else if (lightType === ape.LIGHTTYPE_POINT && light._cookie._cubemap) {
                            usesCookie = true;
                            usesCookieNow = true;
                        }
                    }
                    ...
                    if (light._falloffMode === ape.LIGHTFALLOFF_LINEAR) {
                ...
                if (light.castShadows && !options.noShadow) {
                    ...
                    var evsmExp;
                    if (light._shadowType === ape.SHADOW_VSM8) {
                        shadowReadMode = "VSM8";
                        evsmExp = "0.0";
                    } else if (light._shadowType === ape.SHADOW_VSM16) {
                        shadowReadMode = "VSM16";
                        evsmExp = "5.54";
                    } else if (light._shadowType === ape.SHADOW_VSM32) {
                    ...
                    } else if (light._shadowType === ape.SHADOW_PCF5) {
                    ...
                    if (shadowReadMode !== null) {
                        if (lightType === ape.LIGHTTYPE_POINT) {
                            ...
                            if (lightType === ape.LIGHTTYPE_SPOT) shadowReadMode = "Spot" + shadowReadMode;
        ...
        code += chunks.endPS;
        if (options.blendType === ape.BLEND_NORMAL || options.blendType === ape.BLEND_ADDITIVEALPHA || options.alphaToCoverage) {
            code += chunks.outputAlphaPS;
        } else if (options.blendType === ape.BLEND_PREMULTIPLIED) {
