Object.assign(ape, function () {
  ...
  var JSON_ADDRESS_MODE = {
    "repeat": ape.ADDRESS_REPEAT,
    "clamp": ape.ADDRESS_CLAMP_TO_EDGE,
    "mirror": ape.ADDRESS_MIRRORED_REPEAT
  };
  var JSON_FILTER_MODE = {
    "nearest": ape.FILTER_NEAREST,
    "linear": ape.FILTER_LINEAR,
    "nearest_mip_nearest": ape.FILTER_NEAREST_MIPMAP_NEAREST,
    "linear_mip_nearest": ape.FILTER_LINEAR_MIPMAP_NEAREST,
    "nearest_mip_linear": ape.FILTER_NEAREST_MIPMAP_LINEAR,
    "linear_mip_linear": ape.FILTER_LINEAR_MIPMAP_LINEAR
  };
  ...
  var _legacyDdsLoader = function (url, data, graphicsDevice) {
            ...
            var ext = ape.path.getExtension(url).toLowerCase();
            ...
            if (isFourCc) {
                if (fcc === FCC_DXT1) {
                    format = ape.PIXELFORMAT_DXT1;
                    compressed = true;
                } else if (fcc === FCC_DXT5) {
                    format = ape.PIXELFORMAT_DXT5;
                    compressed = true;
                } else if (fcc === FCC_FP32) {
                    format = ape.PIXELFORMAT_RGBA32F;
                    floating = true;
                } else if (fcc === FCC_ETC1) {
                    format = ape.PIXELFORMAT_ETC1;
                    compressed = true;
                    etc1 = true;
                } else if (fcc === FCC_PVRTC_2BPP_RGB_1 || fcc === FCC_PVRTC_2BPP_RGBA_1) {
                    format = fcc === FCC_PVRTC_2BPP_RGB_1 ? ape.PIXELFORMAT_PVRTC_2BPP_RGB_1 : ape.PIXELFORMAT_PVRTC_2BPP_RGBA_1;
                    compressed = true;
                    pvrtc2 = true;
                } else if (fcc === FCC_PVRTC_4BPP_RGB_1 || fcc === FCC_PVRTC_4BPP_RGBA_1) {
                    format = fcc === FCC_PVRTC_4BPP_RGB_1 ? ape.PIXELFORMAT_PVRTC_4BPP_RGB_1 : ape.PIXELFORMAT_PVRTC_4BPP_RGBA_1;
                    compressed = true;
                    pvrtc4 = true;
                }
            } else {
            ...
            if (!format) {
                ...
                // #endif
                texture = new ape.Texture(graphicsDevice, {
                    width: 4,
                    height: 4,
                    format: ape.PIXELFORMAT_R8_G8_B8
                });
            ...
            var texOptions = {
                // #ifdef PROFILER
                profilerHint: ape.TEXHINT_ASSET,
                // #endif
                width: width,
                height: height,
                format: format,
            ...
            texture = new ape.Texture(graphicsDevice, texOptions);
            if (isCubemap) {
                texture.addressU = ape.ADDRESS_CLAMP_TO_EDGE;
                texture.addressV = ape.ADDRESS_CLAMP_TO_EDGE;
            }
  ...
  Object.assign(TextureHandler.prototype, {
    load: function (url, callback) {
      ...
      var ext = ape.path.getExtension(urlWithoutParams).toLowerCase();
      if (ext === '.dds' || ext === '.ktx') {
          var options = {
              cache: true,
          ...
          if (self.crossOrigin !== undefined && ape.ABSOLUTE_URL.test(url.original)) {
          ...
          image.onerror = function (event) {
              callback(ape.string.format("Error loading Texture from: '{0}'", url.original));
          };
          ...
          if (blobStart >= 0) {
              ...
              // Call error callback with details.
              image.onerror = function (event) {
                  callback(ape.string.format("Error loading Texture from: '{0}'", url));
              };
              ...
              setTimeout(function () {
                  callback(ape.string.format("Error loading Texture: format not supported: '{0}'", ext));
              }, 0);
          }
      }
    },
    open: function (url, data) {
      ...
      if ((data instanceof Image) || (data instanceof HTMLImageElement)) { // PNG, JPG or GIF
          ...
          format = (ext === ".jpg" || ext === ".jpeg") ? ape.PIXELFORMAT_R8_G8_B8 : ape.PIXELFORMAT_R8_G8_B8_A8;
          texture = new ape.Texture(this._device, {
              // #ifdef PROFILER
              profilerHint: ape.TEXHINT_ASSET,
              // #endif
              width: img.width,
          ...
          if (LEGACY && ext === '.dds') {
              ...
              switch (ext) {
                  case '.dds':
                      textureData = new ape.DdsParser(data);
                      break;
                  case '.ktx':
                      textureData = new ape.KtxParser(data);
                      break;
                  case '.pvr':
              ...
              texture = new ape.Texture(this._device, {
                  ...
                  profilerHint: ape.TEXHINT_ASSET,
                  // #endif
                  addressU: textureData.cubemap ? ape.ADDRESS_CLAMP_TO_EDGE : ape.ADDRESS_REPEAT,
                  addressV: textureData.cubemap ? ape.ADDRESS_CLAMP_TO_EDGE : ape.ADDRESS_REPEAT,
