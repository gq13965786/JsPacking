describe('ape.BundleRegistry', function () {
  beforeEach(function () {
    this.loader = sinon.fake();
    this.assets = new ape.AssetRegistry(this.loader);
    this.bundles = new ape.BundleRegistry(this.assets);
  });
  ...
  it('bundle asset is added to the bundle registry', function () {
    var asset = new ape.Asset('bundle', 'bundle', null, { assets: []});
  ...
  it('listBundlesForAsset() returns null for assets not in bundles', function () {
    var asset = new ape.Asset('asset', 'text', {
      url: 'text.txt'
    });
    this.assets.add(asset);

    var notInBundle = new ape.Asset('asset', 'text', {
      url: 'text2.txt'
    });
    this.assets.add(notInBundle);

    var bundleAsset = new ape.Asset('bundle', 'bundle', null, {
      assets: [asset.id]
    });
  ...
  it('hasUrl() returns false for url not in bundle', function () {
    expect(this.bundles.hasUrl('invalid.txt')).to.equal(false);
  });
  ...
  it('hasUrl() returns true for all font asset urls', function () {
    var asset = new ape.Asset('asset', 'font', {
      url: 'test.png'
    },{
      info: {
        maps: [{
    ...
    expect(this.bundles.hasUrl('test1.png')).to.equal(true);//font
  });
  it('hasUrl() returns false for after asset is removed', function () {
    var asset = new ape.Asset('asset', 'text', {
      url: 'text.txt'
    });
    this.assets.add(asset);
  ...
  //callback
  it('loadUrl() calls callback if bundle loaded', function () {

  });
  it('loadUrl() calls callback if bundle is loaded later', function () {


  });
  it('loadUrl() calls callback if other bundle that contains the asset is loaded later', function () {


  });
  it('loadUrl() calls callback with error if bundle that contains the asset is removed', function () {


  });
  it('loadUrl() calls callback if bundle fails to load but another bundle that contains the asset is loaded later', function () {


  });
  it('loadUrl() calls callback with error if bundle fails to load', function () {});
