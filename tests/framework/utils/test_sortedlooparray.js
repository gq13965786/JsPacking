describe("ape.SortedLoopArray", function () {
  var arr;

  beforeEach(function () {
    arr = new ape.SortedLoopArray({sortBy: 'priority'});
  });
  afterEach(function () {
    arr = null;
  });
  ...
  it('insert() increase length', function () {
    expect(arr.length).to.equal(0);
    arr.insert({priority: 1});
    expect(arr.length).to.equal(1);
  });
  it('append() increase length', function () {
    expect(arr.length).to.equal(0);
    arr.append({priority: 1});
    expect(arr.length).to.equal(1);
  });
  it('remove() decrease length', function () {
    var item = { priority: 1};
    expect(arr.length).to.equal(0);
    arr.insert(item);
    expect(arr.length).to.equal(1);
