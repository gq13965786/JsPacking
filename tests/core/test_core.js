describe("core.js testing", function() {
  it('type', function () {
    var types = [
      null,
      1,
    ...
    for (index = 0; index < types.length; index++) {
      expect(ape.type(types[index])).to.equal(expected[index]);
    }
  ...
  it('isDefined', function () {
      ...
      expect(ape.isDefined(a)).to.equal(false);
      expect(ape.isDefined(b)).to.equal(true);
      expect(ape.isDefined(c)).to.equal(true);
      expect(ape.isDefined(d)).to.equal(true);
  });
});
